...
<div style="width: 100%;    text-align: center;    padding: 10px 0px;  color:black;  border-bottom: 2px solid rgba(0, 0, 0, 0.5);    margin-bottom: 10px; " id="encounterHeader">
    <security:oscarSec roleName="<%=roleName$%>" objectName="_newCasemgmt.doctorName" rights="r">
   <!-- MRP        <span style=""><bean:message key="oscarEncounter.Index.msgMRP"/>&nbsp;&nbsp; -->
    ...
    <span class="Header" >
        ...
        %>
        <a href="#" style="color:black;" onClick="popupPage(700,1000,'<%=winName%>','<c:out value="${ctx}"/><%=url%>'); return false;" title="<bean:message key="provider.appointmentProviderAdminDay.msgMasterFile"/>"><%=bean.patientLastName %>, <%=bean.patientFirstName%></a><span>Sex: </span><%=bean.patientSex%><span>Age: </span><%=bean.patientAge%><span>Tel: </span><oscar:phrverification demographicNo="<%=demoNo%>"><bean:message key="phr.verification.link"/></oscar:phrverification> <%=bean.phone%> 
		<span id="encounterHeaderExt"></span>
		<security:oscarSec roleName="<%=roleName$%>" objectName="_newCasemgmt.apptHistory" rights="r">
		<a href="javascript:popupPage(400,850,'ApptHist','<c:out value="${ctx}"/>/demographic/demographiccontrol.jsp?demographic_no=<%=bean.demographicNo%>&amp;<%=bean.patientLastName.replaceAll("'", "\\\\'")%>&amp;first_name=<%=bean.patientFirstName.replaceAll("'", "\\\\'")%>&amp;orderby=appointment_date&amp;displaymode=appt_history&amp;dboperation=appt_history&amp;limit1=0&amp;limit2=25')" style="font-size: 14px;text-decoration:none;" title="<bean:message key="oscarEncounter.Header.nextApptMsg"/>"><span style="color:black;"><bean:message key="oscarEncounter.Header.nextAppt"/> <oscar:nextAppt demographicNo="<%=bean.demographicNo%>"/></span></a>
		</security:oscarSec>
